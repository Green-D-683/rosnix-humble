...
buildAmentCmakePackage (finalAttrs: {
  ...
  propagatedNativeBuildInputs = rosSystemPackages.getPackages { forNativeBuildInputs = [ "libblosc-dev" "libboost-iostreams-dev" "libboost-thread-dev" "tbb" "zlib" ]; };
  buildInputs = [ ament-cmake ament-cmake-vendor-package ] ++ rosSystemPackages.getPackages { forBuildInputs = [ "git" ]; };
  propagatedBuildInputs = rosSystemPackages.getPackages { forBuildInputs = [ "libblosc-dev" "libboost-iostreams-dev" "libboost-thread-dev" "tbb" "zlib" ]; };
  passthru.sources = mkSourceSet (sources: {
    "openvdb_vendor" = substituteSource {
      src = fetchgit {
        ...
        rev = "8cb38de5ce5d10b8afb5a25556fdf122aae02367";
        hash = "sha256-iwmtFE+vWdJvogEoitJ82fnyLK8mx37bb5rrYOtWZGE=";
      };
      substitutions = [
        {
          path = "CMakeLists.txt";
